...
def main():
    ...
    if FLAGS.mode == 'kmeans':
        ...
        for k in k_values:
            ...
            dataset_cluster = clusteringNH.k_means_over_instances(copy.deepcopy(
                dataset), FLAGS.usevariables, k, 'default', 20, 10)
    ...
    if FLAGS.mode == 'kmediods':
        ...
        for k in k_values:
            ...
            dataset_cluster = clusteringNH.k_medoids_over_instances(copy.deepcopy(
                dataset), FLAGS.usevariables, k, 'default', 20, n_inits=10)
            silhouette_score = dataset_cluster['silhouette'].mean()
            print(f'silhouette = {silhouette_score}')
            silhouette_values.append(silhouette_score)
        ...
        dataset_kmed = clusteringNH.k_medoids_over_instances(copy.deepcopy(dataset), FLAGS.usevariables, k, 'default', 20, n_inits=50)
        DataViz.plot_clusters_3d(dataset_kmed, FLAGS.usevariables, 'cluster', ['label'])
        DataViz.plot_silhouette(dataset_kmed, 'cluster', 'silhouette')
        util.print_latex_statistics_clusters(dataset_kmed, 'cluster', FLAGS.usevariables, 'label')
    ...
    if FLAGS.mode == 'agglomerative':
        ...
        for k in k_values:
            print(f'k = {k}')
            dataset_cluster, l = clusteringH.agglomerative_over_instances(dataset, FLAGS.usevariables, k, 'euclidean', use_prev_linkage=True, link_function='ward')
            silhouette_score = dataset_cluster['silhouette'].mean()
            print(f'silhouette = {silhouette_score}')
            silhouette_values.append(silhouette_score)
            if k == k_values[0]:
    ...
    if FLAGS.mode == 'final':
        ...
        dataset = clusteringNH.k_means_over_instances(dataset, FLAGS.usevariables, FLAGS.k, 'default', 50, 50)
        DataViz.plot_clusters_3d(dataset, FLAGS.usevariables, 'cluster', ['label'])
        DataViz.plot_silhouette(dataset, 'cluster', 'silhouette')
        util.print_latex_statistics_clusters(
            dataset, 'cluster', FLAGS.usevariables, 'label')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--k', type=int, default=6,
                        ...
                        ")
    parser.add_argument('--usevariables', type=str, nargs='+', default=['acc_phone_x', 'acc_phone_y', 'acc_phone_z'],
                        help='Selected variables to analyse.')
