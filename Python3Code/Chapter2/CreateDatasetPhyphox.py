...
class CreateDatasetPhyphox:
    ...
    def add_numerical_dataset(self, file, timestamp_col, value_cols, aggregation='avg', prefix=''):
        ...
        # Renaming column names to match the crowdsignals dataset
        dataset.columns = [timestamp_col] + value_cols

        # Convert timestamps to dates
        start_time = pd.to_datetime(
            pd.read_csv(self.base_dir / 'meta/time.csv', skipinitialspace=True)['system time text'][0])
        for i, stamp in enumerate(dataset[timestamp_col]):
            dataset.loc[i, timestamp_col] = start_time + DateOffset(seconds=stamp)
